...
void main() async {
  var config = File('./example/contract/carrot/carrot');
  var content = config.readAsBytesSync();
  var data = listToHex(content);
  var txHash = await sendBinaryData(BigInt.from(8000 * UnitCkb), data);
  ...
  txHash = await callBinaryWithData(
...
Future<String> sendBinaryData(BigInt capacity, String data) async {
...
Future<String> callBinaryWithData(BigInt capacity, Script type, CellDep cellDep,
