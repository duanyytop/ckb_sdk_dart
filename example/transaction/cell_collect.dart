...
class CellCollector {
  ...
  Future<CollectResult> collectInputs(List<String> addresses, Transaction tx,
      BigInt feeRate, int initialLength) async {
    var lockHashes = [];
      ...
      while (fromBlockNumber <= toBlockNumber &&
        ...
        for (var cellOutputWithOutPoint in cellOutputList) {
          if (skipDataAndType) {
            ...
            if ((outputsDataContent.isNotEmpty && '0x' != outputsDataContent) ||
  ...
  Future<BigInt> getCapacityWithAddress(String address) async {
    return await getCapacityWithLockHash(
        AddressParser.parse(address).script.computeHash());
  }
    ...
    while (fromBlockNumber <= toBlockNumber) {
      ...
      if (cellOutputs != null && cellOutputs.isNotEmpty) {
        for (var output in cellOutputs) {
          if (skipDataAndType) {
            var cellWithStatus = await api.getLiveCell(
                outPoint: output.outPoint, withData: true);
            var outputsDataContent = cellWithStatus.cell.data.content;
            var cellOutput = cellWithStatus.cell.output;
            if ((outputsDataContent.isNotEmpty && '0x' != outputsDataContent) ||
                cellOutput.type != null) {
