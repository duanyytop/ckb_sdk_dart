...
class CollectUtils {
  ...
  Future<CollectResult> collectInputs(
      List<String> addresses, Transaction transaction, BigInt feeRate, int initialLength) async {
  ...
  List<CellOutput> generateOutputsNoChange(List<Receiver> receivers) {
    var cellOutputs = <CellOutput>[];
    for (var receiver in receivers) {
      var addressParseResult = AddressParser.parse(receiver.address);
      cellOutputs.add(CellOutput(capacity: bigIntToHex(receiver.capacity), lock: addressParseResult.script));
